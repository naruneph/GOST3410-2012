...
bool Z_256_l(uint8_t* t, uint8_t* q){
	for(int i = 31; i >= 0; i--){
		if(t[i]>q[i]){
			return false;
		}
...
bool Z_512_l(uint8_t* t, uint8_t* q){
	for(int i = 63; i >= 0; i--){
		if(t[i]>q[i]){
			return false;
		}
...
void gen_priv_key_256(uint8_t *d, struct paramset *pset){
	uint8_t t[32]; 
	do{
	 ...
	 // 	for(int i =0; i<32; i++){
		...
		// }
		// cout<<endl;
//	 	cout<<Z_256_iszero(t)<< Z_256_l(t, pset->q);

	}while(Z_256_iszero(t) || !Z_256_l(t, pset->q));

	memcpy(d, t, sizeof(d));
}

void gen_priv_key_512(uint8_t *d, struct paramset *pset){
	uint8_t t[64]; 
	do{
		rand_bytes(t, sizeof (t));
	}while(Z_512_iszero(t) || Z_512_l(t, pset->q));
	memcpy(d, t, sizeof(d));
}
...
void make_exact_size(vector<uint8_t> &vect, int sz){
	for(int i=0; i<sz; i++){
		vect.push_back(0x00);
	}
}
...
int main(int argc, char** argv){
	try{
		...
		if(state["pset"] == "SetA"){
		 	make_exact_size(d, 32);
		 	gen_priv_key_256(&d[0], &SetA);
		} else {
		 	make_exact_size(d, 64);
		 	gen_priv_key_512(&d[0], &SetC);
		}

		reverse(d.begin(),d.end()); // теперь в d[0] старший байт

		fwrite(&d[0],1,d.size(), output_file);
